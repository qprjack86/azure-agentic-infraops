"""
Architecture Diagram: pci-dss-gw Payment Gateway
================================================
...
Generate:
    cd agent-output/pci-dss-gw && python3 03-des-diagram.py
"""
...
graph_attr = {
    ...
    "fontsize": "18",
    "dpi": "150",
    "label": "pci-dss-gw: PCI-DSS Payment Gateway\nswedencentral | 10,000 TPS | 99.99% SLA",
    "labelloc": "t",
}
...
with Diagram(
    ...
    with Cluster("Global Edge Services", graph_attr=cluster_edge):
        ...
        front_door = FrontDoors("fd-pci-dss-gw-prod\nFront Door Premium\n+ WAF (OWASP 3.2)")
    ...
    with Cluster("Shared Services", graph_attr=cluster_monitor):
        ...
        log_analytics = LogAnalyticsWorkspaces("log-pci-dss-gw-prod\n1-year retention")
        app_insights = ApplicationInsights("appi-pci-dss-gw-prod\nDistributed Tracing")
    ...
    with Cluster("Hub VNet (10.0.0.0/16)", graph_attr=cluster_hub):
        with Cluster("snet-firewall (10.0.1.0/24)", graph_attr=cluster_style):
            az_firewall = Firewall("fw-pci-dss-gw-prod\nFirewall Premium\n(IDPS enabled)")
    ...
    with Cluster("Spoke VNet (10.1.0.0/16)", graph_attr=cluster_spoke):
        ...
        with Cluster("snet-apim (10.1.0.0/24)", graph_attr=cluster_style):
            apim = APIManagement("apim-pci-dss-gw-prod\nAPIM Premium\n(VNet-integrated)")

        # --- CDE Subnet (PCI-DSS) ---
        with Cluster("snet-aks-cde (10.1.1.0/24)\nPCI-DSS Cardholder Data Environment", graph_attr=cluster_cde):
            aks = KubernetesServices("aks-pci-dss-gw-prod\nAKS Standard\nD8s_v5 × 3-20 nodes")
            acr = ContainerRegistries("acr-pci-dss-gw-prod\nACR Premium\n(geo-replicated)")

        # --- Messaging Subnet ---
        with Cluster("snet-messaging (10.1.2.0/24)", graph_attr=cluster_style):
            service_bus = ServiceBus("sb-pci-dss-gw-prod\nService Bus Premium\n1 MU")

        # --- Data Subnet ---
        with Cluster("snet-data (10.1.3.0/24)\nPrivate Endpoints Only", graph_attr=cluster_data):
            postgresql = DatabaseForPostgresqlServers("psql-pci-dss-gw-prod\nPostgreSQL Flex\nE16s v5 (16 vCores)\nZone-redundant HA")
            cosmos = CosmosDb("cosmos-pci-dss-gw-prod\nCosmos DB NoSQL\nAutoscale 10-50K RU/s\nSession consistency")
            keyvault = KeyVaults("kv-pci-dss-gw-prod\nKey Vault Premium\n(HSM-backed)")
